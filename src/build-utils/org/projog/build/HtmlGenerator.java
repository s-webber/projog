...
/**
 * Produces all html pages that make up the website.
 * <p>
...
public class HtmlGenerator {
   ...
   /**
    * Generate all html pages that make up the website.
    */
   ...
   /**
    * Produce pages that make up the website that are not included in the manual's "table of contents".
    * <p>
   ...
   private static void produceTableOfContents(List<TableOfContentsEntry> entries) throws Exception {
      ...
      try {
         ...
         for (TableOfContentsEntry next : entries) {
            if (next.isHeader()) {
               bw.write(next.index + " " + next.title + "<br>" + LINE_BREAK);
            } else {
               if (next.isSubSection()) {
                  bw.write("&nbsp;&nbsp;&nbsp; ");
               }
               bw.write(next.index);
               bw.write(" <a href=\"" + next.fileName + "\">" + next.title + "</a><br>" + LINE_BREAK);

               File sourceFile = new File(WEB_SRC_DIR, next.fileName);
               if (!sourceFile.exists()) {
                  sourceFile = new File(DOCS_OUTPUT_DIR, next.fileName);
               }
            }
         }
         bw.write("</span>");
   ...
   private static void addPreviousAndNextLinks(TableOfContentsEntry entry) {
      ...
      outputLink(entry.previous, linksHtml);
      linksHtml.append("</div><div class=\"next\">");
      outputLink(entry.next, linksHtml);
      ...
      contents.insert(0, "<hr><h2>" + entry.index + " " + entry.title + "</h2>" + LINE_BREAK + "<div class=\"section-content\">" + LINE_BREAK);
      contents.insert(0, linksHtml);
      contents.append("</div><hr>");
      contents.append(linksHtml);
      addHeadersAndFooters(entry.fileName, entry.title, contents);
   }

   private static File getFile(TableOfContentsEntry entry) {
      File f = new File(WEB_SRC_DIR, entry.fileName);
      if (!f.exists()) {
         f = new File(DOCS_OUTPUT_DIR, entry.fileName);
      }
      return f;
   }
